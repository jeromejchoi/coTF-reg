...
# Load your data
data1 = pd.read_csv('/ua/Dissertation_JC/Oligo/Step6/ROSMAP_validation/ROSMAP_oligo_mc_TF.csv',header=0)
data2 = pd.read_csv('/ua/Dissertation_JC/Oligo/Step6/ROSMAP_validation/ROSMAP_oligo_mc_TG.csv',header=0)

df3 = pd.concat([data1.iloc[:, :-2], data2.iloc[:,389]], axis=1)
...
np.savetxt(path + "best_actual_values.csv", np.array(best_actual_values), delimiter=",")
np.savetxt(path + "best_predicted_values.csv", np.array(best_predicted_values), delimiter=",")
